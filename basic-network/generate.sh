...
configtxgen -profile ThreeOrgsChannel -outputCreateChannelTx ./config/channel.tx -channelID $CHANNEL_NAME
if [ "$?" -ne 0 ]; then
...
configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./config/GDPRMSPanchors.tx -channelID $CHANNEL_NAME -asOrg GDPRMSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for GDPRMSP..."
  exit 1
fi
